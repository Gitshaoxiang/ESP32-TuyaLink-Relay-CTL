											  ...
											  char* clientID, char* username, char* password)
{
    ...
    hmac_sha256_once((const uint8_t *)deviceSecret, (uint8_t *)passward_stuff, slen, digest);
    for (i = 0; i < 32; i++) {
...
static void mqtt_client_message_cb(void* client, uint16_t msgid, const mqtt_client_message_t* msg, void* userdata)
{
	...
	tuya_link_thing_message_parse(msg->topic, (const char *)msg->payload, msg->length, &out_message);
...
int tuyalink_message_send(tuya_mqtt_context_t* context, tuyalink_message_t* message)
{
	...
	uint16_t mqmsgid = mqtt_client_publish(context->mqtt_client, topic_stuff, (uint8_t *)payload, payload_length, MQTT_QOS_0);
	system_free(payload);
	if (mqmsgid <= 0) {
